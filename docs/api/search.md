...
`GET /api/search` accepts `q`, `author`, `date_from`, `date_to`, `sort=relevance|date`, `direction=asc|desc`, `limit=1..50` and `offset=0..10000`. Elasticsearch searches comment text and author fields, supports fuzzy matching, applies date filters and returns text highlights. Responses include `next_offset`; a null value marks the final page.
